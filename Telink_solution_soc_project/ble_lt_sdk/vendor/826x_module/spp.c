...
extern int	mcu_uart_data_flg;
extern int	module_uart_data_flg;
extern u32  mcu_wakeup_module_tick;
...
int hci_send_data (u32 h, u8 *para, int n)
{
	...
	if(!mcu_uart_data_flg)
	{ //UART idle, new data is sent
		GPIO_WAKEUP_MODULE_LOW;  //Notify module that there is data here
		mcu_wakeup_module_tick = clock_time() | 1;
		mcu_uart_data_flg = 1;
	}
#endif
...
int tx_to_uart_cb (void)
{
	...
	if (p && !uart_tx_is_busy ())
	{
...
#if (BLE_MCU_INDICATE_DATA_TO_MODULE)
		//If the module side is designed to have low power consumption and the mcu has data to pull up
		//the GPIO_WAKEUP_MODULE will only wake up the MODULE, then you need to consider whether MODULE needs a
		...
		{
			while(!clock_time_exceed(mcu_wakeup_module_tick, 100));
		}
#endif
