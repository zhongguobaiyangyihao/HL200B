...
static u8                UART_receive_over_judge_cnt = 0;

//const u8                 module_cmd_AT[] = "AT\r\n";
const u8                 module_cmd_AT[] = "AT+HTTPPARA=\"TRACELEVEL\",0\r\n";
/************************************************************************/
...
static void module_receive_data_clear(void);
void module_receive_data_finish_judge(void);
void module_receive_data_handler(void);
/************************************************************************
...
void module_receive_data_finish_judge(void)
{
	if(Flag.is_uart_receive_start)
	{
		...
		if(UART_receive_over_judge_cnt>=(1000/GPRS_GPS_MODULE_COMMUNICATE_INTERVAL))
		{
...
void module_receive_data_handler(void)
{
	...
	{
		printf("uart auto rec: ");printf("%s",(char *)UART_RX_BUF_FROMMODULE);printf("\r\n");
		module_receive_data_clear();
	}
}
/************************************************************************
 * module_receive_data_clear
...
static void module_receive_data_clear(void)
{
	...
	memset(UART_RX_BUF_FROMMODULE,0,UART_RX_BUF_FROMMODULE_SIZE);
}
/************************************************************************
 * module_pwron
************************************************************************/
...
static void module_turnon_schedule(void)
{
	...
	switch(module_turnon_schedule)
	{
		case module_turnon_pwron:
		{
            ...
            if(waiting_time >= WAIT_TIME_SHORT)
			{
				waiting_time = 0;
...
//						printf("uart rec: ");foreach(i, UART_RX_BUF_FROMMODULE_CNT){PrintHex(*((u8*)UART_RX_BUF_FROMMODULE+i));}printf("\r\n");
						printf("uart rec: ");printf("%s",(char *)UART_RX_BUF_FROMMODULE);printf("\r\n");
